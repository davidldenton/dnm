---
title: "Annual sales estimation of an online black market"
author: "David Denton"
date: "5/18/2017"
...
A few simple charts can help indicate which products are most commonly purchased at DNM. The following bar chart illustrates how much each product category contributes to the site's overall annual sales.
...
By itself, the site's top product, LSD, generates **`r round((dat %>% filter(product == "LSD") %>% mutate(avg_sales = mean(c(avg_daily_sales, max_avg_daily_sales))) %>% summarise(dc_avg = sum(avg_sales)*365))/(mean(c(sum_dat$total_annual_sales, sum_dat$max_total_annual_sales)))*100,1)
`%** of all revenues.
